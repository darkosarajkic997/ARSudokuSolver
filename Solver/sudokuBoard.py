...
class SudokuBoard:
    ...
    def __init__(self, board=const.TEST_BOARD, dim_x=const.BOARD_DIM_X, dim_y=const.BOARD_DIM_Y, input_type='string',cell_dim_x=const.CELL_DIM_X,cell_dim_y=const.CELL_DIM_Y):
        ...
        if(input_type=='string'):
            self.board = np.array([int(digit) for digit in board]).reshape(self.dim_x, self.dim_y)
        else:
            self.board=board
        self.markup = np.tile(
    ...
    def remove_initial_values_from_markup(self):
        ...
        for index in range(0, nonzero_indices[0].size):
            ...
            _=self.remove_value_from_markup(row, column, self.board[row,column])
    ...
    def remove_value_from_markup(self, row_index, column_index, value):
        ...
        removed_markups+=self.remove_value_from_markup_column(column_index, value)
    ...
    def solve(self):
        ...
        next_guess_index = np.where(self.markup_depth==np.amin(self.markup_depth[np.where(self.markup_depth>0)]))
        row_index = next_guess_index[0][0]
        column_index = next_guess_index[1][0]

        values_for_check = self.markup[row_index, column_index,:][self.markup[row_index, column_index, :] > 0]

        for value in values_for_check:
            place_markups,removed_markups=self.insert_value_in_place(row_index, column_index, value)
            if(self.check(row_index,column_index) and self.solve()):
                return True
            else:
                self.remove_value_from_place(row_index, column_index, value, removed_markups, place_markups)
    ...
    def insert_value_in_place(self, row_index, column_index, value):
        ...
        removed_markups=self.remove_value_from_markup(row_index, column_index, value)
    ...
    def remove_value_from_place(self, row_index, column_index, value, removed_markups, place_markups):
        ...
        self.markup_depth[row_index,column_index]=np.count_nonzero(place_markups)
